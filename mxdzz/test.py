...
def get_slide_pix(img_arr: np.ndarray):
    ...
    for lc in range(0, 255, 10):
        ...
        if len(left_where) > 50:
            ...
            break
    if not y0:
        # img.save(f"unfind_slide_{t}.png")
    ...
    delta_arr_cut[np.where(delta_arr_cut < 0)] = 0
    delta_arr_cut = delta_arr_cut.astype(np.uint8)
    Image.fromarray(delta_arr_cut).save("left.png")
    right_img_slide_arr_cut = img_slide_arr_cut[:15, :]
    delta_right_arr_cut = (
        right_img_slide_arr_cut[:, 1:] - right_img_slide_arr_cut[:, :-1]
    )
    delta_right_arr_cut[np.where(delta_right_arr_cut < 0)] = 0
    delta_right_arr_cut = delta_right_arr_cut.astype(np.uint8)
    Image.fromarray(delta_right_arr_cut).save("right.png")
    ...
    for lc in range(0, 200, 10):
        ...
        if len(right_pix_where) > 0:
            ...
            if right_pix < 145:
                return right_pix - 66
            # img.save(f"find_slide_{y0}_{y1}_{pix}_{t}.png")
            # return pix
    # img.save(f"unfind_slide_{y0}_{y1}_{t}.png")
    return np.random.randint(10, 80)


# for file in os.listdir("dist"):
#     if file.endswith(".png"):
#         img = Image.open(f"dist/{file}")
#         img_arr = np.array(img)
#         print(get_slide_pix(img_arr))
img = Image.open("dist/slide/unfind_slide_107_160_1699752842.png")
print(get_slide_pix(np.array(img)))