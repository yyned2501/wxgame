...
真机连跑 **3 次**完整闭环（13:05:31 / 13:08:13 / 13:12:21），后两次都落在 `CLAIM_GAP = 120s` 之后：
...
三次读到的药丸缩窄完全一致（`270 -> 209`，同一个广告源），点[关闭]后都是 **3s** 内认出已离开广告页 ->
`done` 当帧交回路由，没有一次 `giveup`、没有一次 `Traceback`，广告窗口期间路由表一次都没被调用。
