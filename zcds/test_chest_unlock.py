...
from pages.lobby import (CHEST_SLOTS, CHEST_BADGE_MIN, LobbyPage,       # noqa: E402
...
def main():
    ...
    print('')
    print('[4] 行为: 没角标的[点击解锁]一律不点, 有角标的照点')
    PVP = color_button(img_of(os.path.join(ROOT, 'shots', 'lobby_clean.png')),
                       PVP_BOX, PVP_COLOR, PVP_MIN_PX)
    NO_CHEST = [('shots/live_lobby.png', 'pppa'),           # 3 格白字全无角标 + 1 格[AD]
                ('shots/lobby_mixed_0019.png', 'ap..'),     # 1 格[AD] + 1 格白字无角标
                ('shots_live/tap_hero.png', 'appp'),        # 3 格白字无角标 + 1 格[AD]
                ('shots_live/dbg_002_lobby_081833.png', 'ppap')]
    ...
    for rel, want in NO_CHEST:
        ...
        c = new_ctx(img)
        LobbyPage().act(c)
        check(c.clicks == [PVP], '%-34s 一格都不点 -> 改打对战' % os.path.basename(rel), str(c.clicks))
    ...
    for rel, want, clickable in LIVE:
        ...
        if clickable:
            ...
            check(c.clicks == [PVP],
                  '%-24s 无免费格 -> 真机这帧确实只打了对战' % os.path.basename(rel), str(c.clicks))
    if not got:
        print('     (真机帧都不在, 跳过)')
    ...
    if FAILS:
        ...
        for m in FAILS:
            print('  - ' + m)
        return 1
    print('全部通过 (红角标=此刻能白嫖; 白字但无角标=只能花宝石, 一律不点)')
    return 0
